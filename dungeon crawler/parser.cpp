...
void Parser::parsuj()
{
	if (!linia.empty())
	{
		...
		if (koniec_komendy == wstring::npos) koniec_komendy = linia.length();
		wstring Skomenda = linia.substr(0, koniec_komendy);	//wyodrebniam tekst do spacji
...
void Parser::wyciagnij_argument(int miejsce)
{
	...
	if (!linia.empty())
	{
		...
		if (ch >= '0' && ch <= '9')	//jezeli argument zaczyna sie od cyfry
		{
			...
			if (koniec_liczby != wstring::npos)		//jezeli znaleziono kropke
			{
				wstring liczba = linia.substr(0, koniec_liczby);	//liczbe roboczo zapisujemy jako oddzielny wstring
				...
				case 2:
					...
					break;
				default:
					wcout << "Blad parsera, niepoprawne wywolanie funkcji wyciagnij_argument, przekaz to malpie ktora to pisala." << endl;
				}
		...
		if ((!linia.empty()) && (linia[0] == '\"'))		//jezeli linia zaczyna sie od cudzyslowa
		{
			...
			if (koniec_cytatu != wstring::npos)		//jezeli znaleziono drugi cudzyslow
			{
		...
		else
		{
			...
			if (koniec_argumentu != wstring::npos)
			{
				switch (miejsce)
				{
				case 1:
...
Ekomenda Parser::znajdz(wstring& command)
{
...
KierunekSwiata Parser::znajdzKierunek(wstring& kierunek)
{
	wstring porownawczy = L"";
	if (kierunek[0] == 'n')
	{
		porownawczy = L"north";
